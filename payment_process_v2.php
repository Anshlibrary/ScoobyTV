...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($plan) && !empty($month) && !empty($utr) && !empty($purchase)) {
        $sql = "UPDATE users SET plan_valid_for = ?, plan_amount = ?, txn_id = ?, purchase = ?, plan_expiry = ?, no_of_devices = ? WHERE id = ?";
        if ($stmt = $conn->prepare($sql)) {
            $stmt->bind_param("issssii", $month, $plan, $utr, $purchase, $plan_expiry, $no_of_devices, $user_id);
            if ($stmt->execute()) {
