<?php
error_reporting(E_ALL);  // Report all types of errors (notices, warnings, fatal errors, etc.)
...
$associate_sql = "SELECT fullname, credits, username, user_type, isdisabled FROM Associate WHERE associate_id = ?";
$associate_stmt = $conn->prepare($associate_sql);
if (!$associate_stmt) {
    $_SESSION['statusMessages'][] = "Database error: (" . $conn->errno . ") " . $conn->error;
    header('Location: renew_users.php');
    exit;
}
...
$admin_name =$associate['username'];
$current_credits = $associate['credits']; // Associate's current credits
$user_type = $associate['user_type'];
$isdisabled = $associate['isdisabled'];
// Check if the
        if ($isdisabled) {
            header('Location: signout.php');
            exit();
...
function getExpiredUsers($conn, $admin_name) {
    ...
    $sql = "SELECT id, full_name, email, modified, plan_valid_for, plan_expiry, username, txn_id, jellyfin_status
            FROM users 
            WHERE plan_expiry < NOW() AND txn_id = ?";
...
function updateJellyfinStatus($conn, $userId, $status, $months, $adminName) {
     $sql = "UPDATE users SET jellyfin_status = ?, plan_expiry = DATE_ADD(NOW(), INTERVAL ? MONTH), plan_valid_for =?, updated_by = ? WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("siisi", $status, $months, $months, $adminName, $userId);
    return $stmt->execute();
}
...
function EnableUserByUrl($serverUrl, $apiKey, $userId, $jellyuser, $jellypass) {
    
   $result =  deleteUserInJellyfin($serverUrl, $apiKey, $userId);
...
function EnableUsers($conn, $serverUrl, $apiKey, $usernames, $selectedMonths, $admin_name, $current_credits) {
    $statusMessages = [];
    foreach ($usernames as $username) {
         $months = $selectedMonths[$username]; // Get the selected month for each user
 $required_credits = $months; // 1 credit per month

        // Check if the associate has enough credits
        if ($current_credits < $required_credits) {
            $statusMessages[] = "Not enough credits to activate user '$username'. Required: $required_credits, Available: $current_credits.";
        ...
        if ($dbUserId) {
            ...
            if ($jellyfinUserId) {
                if (EnableUserByUrl($serverUrl, $apiKey, $jellyfinUserId, $username, $dbPassword)) {
                    // Update the Jellyfin status in the database using the database user ID
                    updateJellyfinStatus($conn, $dbUserId, "active - $jellyfinUserId", $months, $admin_name);
                    $statusMessages[] = "User '$username' successfully activated. '$required_credits' credits deducted.";
                    $current_credits -= $required_credits;
                    updateAssociateCredits($conn, $_SESSION['associate_id'], $current_credits);
                } else {
                    $statusMessages[] = "Failed to activate user '$username' via Jellyfin API.";
                }
            } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['disable_users'])) {
    ...
    $selectedMonths = $_POST['months'] ?? []; // Capture selected months for each user
    $statusMessages = EnableUsers($conn, $serverUrl, $apiKey, $usernamesToDisable, $selectedMonths, $admin_name, $current_credits);
    ...
    <div class="d-flex justify-content-between align-items-center">
      ...
      <div class="my-breadcrumb-list">
        <ol > 
          ...
          <li><a href="renew_users.php">Renew_users</a></li>
   ...
   <header id="header" style="padding:0;">
    <div class="container d-flex align-items-center justify-content-between">
      ...
      <nav id="navbar" class="navbar">
        <ul>
          ...
          <li><a class="nav-link scrollto active" href="renew_users.php">Renew users</a></li>
    ...
    <div class="container profile-container mt-0" data-aos="fade-up">
      <div class="section-title mt-0">
        <h2 style="margin-bottom:0;font-size: 26px;padding-bottom:0;">Renew Expired Users</h2>
        <div class="my-3">
...
<?php if (!empty($expiredUsers)): ?>
            ...
            <tbody>
                <?php foreach ($expiredUsers as $user): ?>
                    <tr>
                       ...
                       <select name="months[<?= htmlspecialchars($user['username']) ?>]" class="form-control">
                            <option value="1">Subscription for 1 month</option>
                            <option value="2">Subscription for 2 months</option>
                        </select>
                    </div></td>
    ...
    <button  class="btn btn-buy" id="run_btn">Renew</button>
<?php else: ?>
