...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($plan) && !empty($month) && !empty($utrid) && !empty($purchase)) { // Include purchase in validation
        ...
        if ($stmt = $conn->prepare($sql)) {
            ...
            $stmt->bind_param("issssii", $month, $plan, $utrid, $purchase, $plan_expiry, $no_of_devices, $user_id); // Bind purchase parameter
            ...
            if ($stmt->execute()) {
                ...
                try {
                    ...
                    // Content
                    $mail->isHTML(true);
                    $mail->Subject = 'New Plan Purchase';
                    $mail->Body = "
