...
<?php if (!empty($expiredUsers)): ?>
    <form method="post" id="users_disable">
       ...
       <button type="submit" name="disable_users" class="btn btn-buy" id="submit_btn">Disable Selected Users</button>
    </form>
  <script>
    document.getElementById('users_disable').addEventListener('submit', function(e) {
        ...
        .then(data => {
            // Process the response if needed (e.g., display status messages)
        ...
        .catch(error => {
            ...
            // Optionally, re-enable the button on error
            document.getElementById('submit_btn').disabled = false;
        });
    });
</script>
