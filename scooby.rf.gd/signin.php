...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("SELECT id, first_name, allocated_ip FROM users WHERE email = ? AND password = ?");
    ...
    if ($stmt->num_rows > 0) {
        // Fetch user details
        $stmt->bind_result($id, $first_name, $allocated_ip);
        $stmt->fetch();
    ...
    <section id="contact" class="contact section-bg" style="background: linear-gradient(45deg, rgba(59, 59, 59, 0.9) 0%, rgba(32, 32, 32, 0.9) 100%);">
      <div class="container" data-aos="fade-up">
        <div class="section-title">
          <h2 style="margin-bottom:0;">Sign in</h2>
        </div>
        <div class="row">
          <div class="col-lg-12">
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" role="form" class="my-email-form" id="loginForm">
              ...
              <div class="text-center mt-3">
                <a href="#" class="text-blue" data-bs-toggle="modal" data-bs-target="#forgotPasswordModal">Forgot Password?</a>
              </div>
              <div class="text-center">
                <span class="text-dark">Don't have an account?</span><a href="signup.php" class="text-blue"> Sign Up</a>
              </div>
            </form>
          </div>
        </div>
