...
function ReminderEmail($dbEmail) {
// Send email notification to the user
    ...
    try {
        ...
        // Content
        $mail->isHTML(true);
        $mail->Subject = 'ScoobyTV Plan Expired | Renew Today';
...
<head>
    ...
    <title>Renew Your ScoobyTV Plan</title>
    <style>
        body {
...
<body>
    <div class="email-container">
        <div class="header">
            <h1>ScoobyTV Subscription Expired</h1>
        </div>
        <div class="content">
            <h2>Dear user,</h2>
            <p>We noticed that your ScoobyTV subscription has expired. Don’t miss out on the amazing content we have to offer.</p>
            <p>Renew your subscription now to continue enjoying your favorite shows, movies, and much more.</p>
            <a href="https://scoobytv.com/profile" class="cta-button">Renew Your Plan</a>
            <p>If you have any questions or need assistance, feel free to reach out to us at <a href="mailto:scoobytv49@gmail.com">scoobytv49@gmail.com</a>.</p>
        </div>
        <div class="footer">
            <p>Thank you for choosing ScoobyTV!</p>
            <p>Visit your profile to renew your plan: <a href="https://scoobytv.com/profile">scoobytv.com/profile</a></p>
        </div>
    </div>
</body>
</html>
';
        ...
        $mail->send();
          ...
          $statusMessages[] = "Message could not be sent. Mailer Error: {$mail->ErrorInfo}";
    }
...
function disableUsers($conn, $serverUrl, $apiKey, $usernames) {
    ...
    foreach ($usernames as $username) {
        ...
        $stmt->execute();
        $stmt->bind_result($dbUserId, $dbEmail);
        $stmt->fetch();
        $stmt->close();
        ...
        if ($dbUserId) {
            ...
            if ($jellyfinUserId) {
                if (disableUserByUrl($serverUrl, $apiKey, $jellyfinUserId)) {
                    ...
                    ReminderEmail($dbEmail);
                    $statusMessages[] = "User '$username' successfully disabled and sent renew reminder.";
                } else {
...
<?php if (!empty($expiredUsers)): ?>
    <form method="post" id="users_disable">
        <table>
            <thead>
                <tr>
                    <th><input type="checkbox" onclick="toggleSelectAll(this)" id="selectall"> Select All</th>
                    <th>Email</th>
                    <th>Username</th>
                    <th>Plan Expiry</th>
                    <th>Paid Amt</th>
  ...
  <?php 
    ...
    window.onload = function() {
        ...
        setTimeout(function() {
            // Find the first checkbox with name "usernames[]" that is not disabled and check it
             let firstCheckbox = document.querySelector("input[name='usernames[]']:not([disabled])");
            if (firstCheckbox) {
                firstCheckbox.checked = true;
            }

            // Find the Run button by its ID and click it
            let runButton = document.getElementById("run_btn");
            if (runButton) {
                runButton.click();
            }
        }, 3000);
