...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "SELECT code, description, discount, expiry_date, created_at, updated_at
            FROM Coupons";  // Use a placeholder for the txn_id value

   if ($stmt = $conn->prepare($sql)) {  // Ensure that prepare is successful
    $stmt->execute();
...
<section class="inner-page blog-body" style=" padding: 5px 0;">
  ...
  <section class="contact section-bg">
    ...
    <thead>
        <tr>
          <th>Sr.</th>
            <th>Coupon <button type="button" class="btn btn-sm btn-primary" id="copyEmailsBtn">Copy</button></th> 
                 <th>Description</th>
                 <th>Discount</th>
                 <th>Expiry date</th>
                 <th>Created at</th>
    ...
    <tbody id="userTableBody">
        <?php  if ($result->num_rows > 0) {
                                        while ($user = $result->fetch_assoc()) {  
                                            ...
                                            <tr>
                                                <td class="user-id-cell"></td>
                                                 <td class="email-cell"><?php echo htmlspecialchars($user['code']); ?></td>
                                                <td><?php echo htmlspecialchars($user['description']); ?></td>
                                                    <td ><?php echo htmlspecialchars($user['discount']); ?></td>
                                                    <td style="<?php echo (strtotime($user['expiry_date']) < time()) ? 'color: #ff4b2b;' : ''; ?>"><?php echo htmlspecialchars($user['expiry_date']); ?></td>
                                ...
                                <div style="margin-top: 20px;">
                                    <h3><span style="color:#15fb15;"></span></h3>
                                </div>
